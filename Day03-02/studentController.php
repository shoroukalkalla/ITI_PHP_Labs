...
if (isset($_REQUEST['registration'])) {
    ...
    $query = $conn->prepare("insert into students(name, email, password, roomNo, ext, image) values(?, ?, ?, ?, ?, ?)");
    ...
    $query = $conn->query("select * from students where id = {$_REQUEST['view_id']}");
    ...
    $query = $conn->query("delete from students where id = {$_REQUEST['delete_id']}");

    $query->execute();
    header("Location: display.php");
} else if (isset($_REQUEST['edit_id'])) {
    $query = $conn->query("select * from students where id = {$_REQUEST['edit_id']}");
    ...
    foreach ($_REQUEST as $key => $value) {
        if ($value && ($key != "id" || $key != 'edit')) {
            if ($key != 'confirmPassword') {
                if ($key == "password") {
                    $query = $conn->query("update students set password='$password' where id = {$_REQUEST['id']};");

                    $query->execute();
                    continue;
                }
                $query = $conn->query("update students set $key='$value' where id = {$_REQUEST['id']};");
